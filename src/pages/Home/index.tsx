...
import {
  useMarketData,
  useQuoteHomePage,
} from '../../market/endpoints/marketWithTransformer';
...
const Home = () => {
  ...
  return (
    <Container className="flex-1 overflow-y-auto bg-[#eef3f9] px-2.5">
      ...
      <div className="mt-3 grid grid-cols-5 gap-1 overflow-hidden rounded-lg bg-white px-2.5 py-2.5 text-sm font-bold shadow-md shadow-black/5">
        <Link to="/institution" className="flex flex-col items-center">
          <img alt="" src={tab1_menu1} className="h-9 w-9" />
          ...
          >
            ...
            <div className="flex w-1/3 flex-col items-end text-sm">
              <div
                ...
                style={{
                  backgroundColor: getColor(v.rate?.includes('-') ? -1 : 1),
                }}
              >
                {v.rate}%
              </div>
