...
const AddressManagement = () => {
  ...
  const { data, isLoading, refetch } = useAddressList({});
  ...
  const content = useMemo(() => {
    ...
    if (data?.data?.length) {
      return data?.data?.map((v) => (
        <div
          key={v.id}
          className=" mb-4 flex items-center rounded-lg bg-white px-5 py-4 shadow-md shadow-black/5 dark:bg-[#2A2E38]"
        >
          <div className="flex min-w-0 flex-1 flex-col">
            <div className="flex items-center justify-between text-lg text-[#6175AE] dark:text-white">
              <span>{v.symbol}</span>
              <a
                onClick={() => {
                  Dialog.confirm({
            ...
            <div className="mt-1 break-words text-xs text-[#A2A9BC] dark:text-[#AAAAAA]">
              {v.address}
            </div>
    ...
    <Screen
      ...
      right={
        <div className="flex justify-end">
          <Link to="/addresses/add" className="text-[#333] dark:text-white">
            <AddOutline fontSize={24} color="currentColor" />
          </Link>
        </div>
      }
    >
